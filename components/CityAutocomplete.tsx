  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      {isOpen && suggestions.length > 0 && (
        <ul className="absolute z-10 w-full bg-slate-600 border border-slate-500 rounded-md mt-1 max-h-60 overflow-y-auto shadow-lg">
          {suggestions.map((suggestion, index) => (
            <li
              key={index}
              onClick={() => handleSuggestionClick(suggestion)}
              className="px-3 py-2 cursor-pointer text-white hover:bg-slate-500"
            >
              {suggestion}
            </li>
          ))}
