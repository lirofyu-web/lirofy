    ...
    const handleGenerateQrCode = useCallback(async () => {
        try {
            await downloadComponentAsPng(
                ...
                `qrcode-cliente-${customer.name.replace(/\s/g, '_')}.png`
            );
            showNotification('Download do QR Code iniciado.', 'success');
        } catch (error) {
            showNotification('Falha ao gerar QR Code para download.', 'error');
            console.error(error);
        }
