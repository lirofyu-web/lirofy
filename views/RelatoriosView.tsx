...
const RelatoriosView: React.FC<RelatoriosViewProps> = ({ customers, billings, expenses, debtPayments }) => {
  ...
  const stats = useMemo(() => {
    ...
    const revenueMesa = monthlyBillings.filter(b => b.equipmentType === 'mesa' && b.paymentMethod !== 'fiado').reduce((sum, b) => sum + b.valorTotal, 0);
    const revenueJukebox = monthlyBillings.filter(b => b.equipmentType === 'jukebox' && b.paymentMethod !== 'fiado').reduce((sum, b) => sum + b.valorTotal, 0);

    // Debt Generated by Equipment
    const debtGeneratedMesa = monthlyBillings.filter(b => b.equipmentType === 'mesa' && b.paymentMethod === 'fiado').reduce((sum, b) => sum + b.valorTotal, 0);
    const debtGeneratedJukebox = monthlyBillings.filter(b => b.equipmentType === 'jukebox' && b.paymentMethod === 'fiado').reduce((sum, b) => sum + b.valorTotal, 0);
  ...
  }, [stats.debtorCustomers, stats.totalDebt, billings]);

  const handlePrintFinancialReport = useCallback(() => {
    const reportHtml = `
      <html>
        <head>
        ...
        <body>
          ...
          <table class="details-table">
            ...
            <tbody>
              ${stats.monthlyBillings.filter(b => b.equipmentType === 'mesa').map(b => `
                <tr>
          ...
          <table class="details-table">
            <thead><tr><th>Data</th><th>Cliente</th><th>Pgto</th><th class="currency">Valor</th></tr></thead>
            <tbody>
              ${stats.monthlyBillings.filter(b => b.equipmentType === 'jukebox').map(b => `
                <tr>
                  <td>${new Date(b.settledAt).toLocaleDateString('pt-BR')}</td>
                  <td>${b.customerName}</td>
                  <td>${b.paymentMethod}</td>
