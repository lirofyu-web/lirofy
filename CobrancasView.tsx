...
const BillingsList: React.FC<any> = ({ billings, onEdit, onDelete, onShowActions, totalBilled, handleSort, renderSortArrow, getNetBilledAmount, customerMap }) => (
    <>
        {/* Mobile View: Cards */}
        <div className="md:hidden space-y-4 mb-10">
            {billings.length > 0 ? billings.map((billing: Billing) => {
                const customer = customerMap.get(billing.customerId);
                const isDebtSettled = !!(customer && customer.debtAmount === 0 && billing.valorDebitoNegativo && billing.valorDebitoNegativo > 0);
                        ...
                        <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                                {billing.valorDebitoNegativo && billing.valorDebitoNegativo > 0 ? (
                                    isDebtSettled ? (
                                        <p className="font-mono font-bold text-lg text-green-500 dark:text-green-400">
                ...
                <table className="w-full text-sm">
                    <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-100 dark:bg-slate-700/50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-right cursor-pointer" onClick={() => handleSort('valorTotal')}>Valor {renderSortArrow('valorTotal')}</th>
...
const CobrancasView: React.FC<CobrancasViewProps> = ({ 
    ...
    return (
        <>
            ...
            {activeTab === 'billings' && <BillingsList billings={completedBillings} onEdit={onEditBilling} onDelete={setDeletingBilling} onShowActions={onShowActions} totalBilled={totalBilled} handleSort={handleSort} renderSortArrow={renderSortArrow} getNetBilledAmount={getNetBilledAmount} customerMap={customerMap} />}
